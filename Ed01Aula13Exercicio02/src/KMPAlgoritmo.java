public class KMPAlgoritmo {
    ...
    public  String countSequenceOccurrencesInverso(ListaDuplamenteEncadeada list, ListaDuplamenteEncadeada sequence) {
        int count = 0;
        ListaDuplamenteEncadeada sequencias = new ListaDuplamenteEncadeada();
        int[] prefixTable = buildPrefixTable(sequence);
        ...
        while (i < list.tamanho()) {            
            if (Integer.parseInt(list.pegaCelulaAnterior(i).toString()) == Integer.parseInt(sequence.PegaCelula(j).toString())) {
                i++;
                j++;
                if (j == sequence.tamanho()) {    
                    count++;
                    sequencias.Adiciona("Vezes encontrada ordem inversa: " + count + " \nPosição do 1º elemento: " + (51000000 - (i -5)) + " \nPosição do 2º elemento: " + (51000000 - (i -4)) +  " \nPosição do 3º elemento: " + (51000000 - (i -3)) + " \nPosição do 4º elemento: " + (51000000 - (i -2)) + " \nPosição do 5º elemento: " + (51000000 - (i -1)) + " \nPosição do 6º elemento: " + (51000000 - i));
                    // Reiniciar a busca a partir da próxima posição
                    j = prefixTable[j - 1];
                }
            } else {
